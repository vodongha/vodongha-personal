...
public static class AdminProjectsApi
{
    public static void MapAdminProjectsApi(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async (Project item, IDbContextFactory<AppDbContext> dbFactory) =>
        {
            await using AppDbContext db = await dbFactory.CreateDbContextAsync();
            item.Id = 0;
        ...
        group.MapPut("/{id:int}", async (int id, Project item, IDbContextFactory<AppDbContext> dbFactory) =>
        {
            await using AppDbContext db = await dbFactory.CreateDbContextAsync();
            item.Id = id;
        ...
        group.MapPut("/order", async (OrderUpdateRequest req, IDbContextFactory<AppDbContext> dbFactory) =>
        {
            await using AppDbContext db = await dbFactory.CreateDbContextAsync();
