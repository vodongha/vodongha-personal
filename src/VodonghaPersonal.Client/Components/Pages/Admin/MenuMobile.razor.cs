...
public partial class MenuMobile : ComponentBase, IAsyncDisposable
{
    ...
    private List<MenuItem> AllItems => [
        ...
        new("dash-settings",     "/admin/profile",     "bi bi-person-vcard",      Loc.T("Settings")),
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        try
        {
            ...
            if (width > 768) { Nav.NavigateTo("/admin", replace: true); return; }
            _menuOrder = await ApiKeyClient.GetValueAsync(MenuPrefKey) ?? "[]";
    ...
    [JSInvokable]
    public void OnResizedToDesktop() { Nav.NavigateTo("/admin", replace: true); }
    ...
    public sealed record MenuItem(string CardId, string Href, string Icon, string Label, int BadgeCount = 0, string ExtraClass = "");
}
