...
| ORM | Entity Framework Core + `Oracle.EntityFrameworkCore` (ODP.NET), `UseOracleSQLCompatibility(DatabaseVersion19)`. Hybrid `int Id` (internal PK) + `Guid Rid` (external API identifier, stored `RAW(16)`) on all 14 entities; no raw SQL in application code. See **Oracle database notes** below for provider gotchas. |
...
- **Server:** each `Api/AdminXxxApi.cs` exposes `GET /api/admin/{entity}/paged?page=&pageSize=&search=&sortBy=&sortDir=` returning `PagedResult<T>(List<T> Items, int Total)` (DTO in `Shared/DTOs/AdminDtos.cs`). Built with EF Core: `EF.Functions.Like(col.ToUpper(), term.ToUpper())` for case-insensitive search (Oracle has no `ILIKE`), a `switch` on `sortBy` for sorting, then `IQueryable<T>.ToPagedResultAsync(page, pageSize)` (`Api/PagingExtensions.cs`). The original non-paged `GET /` list endpoints are kept.
...
The schema is a **single `InitialOracle` migration** (the pre-Oracle Npgsql migrations were removed during the DB migration). `dotnet ef` requires the Oracle wallet to reach the DB for `database update`; set `TNS_ADMIN` + `ConnectionStrings__DefaultConnection` env and run inside the .NET SDK container (there is no local .NET SDK on the home server — build/EF run in Docker).

## Oracle database notes

Migrated from Neon PostgreSQL to **Oracle Autonomous Database 19c** (shared ADB, schema `VODONGHA_PERSONAL`; same ADB as family-budget/travel-mate). Provider is `Oracle.EntityFrameworkCore` (ODP.NET managed, thin, wallet mTLS). Key provider differences vs PostgreSQL — keep these in mind when editing models/queries:

- **`UseOracleSQLCompatibility(DatabaseVersion19)`** is set in `Program.cs` and `AppDbContextFactory` (ADB is 19c, no native `BOOLEAN`). This maps `bool` → `NUMBER(1)`. Without it the provider emits `BOOLEAN` → `ORA-00902`.
- **Empty string = NULL.** Oracle cannot store `''` — it becomes `NULL`. So any `string` property that can be empty **must be nullable (`string?`)**, otherwise ODP.NET throws `ORA-50032` when reading the NULL back. Already-nullable optional fields are fine; when adding a new optional string field, declare it `string?`.
- **`Guid` → `RAW(16)`** using .NET `Guid.ToByteArray()` byte order (mixed-endian). Any external data load must convert accordingly.
- **Long text → `NCLOB`** (configured via `HasColumnType("NCLOB")` in `OnModelCreating` for Content/Summary/Description/Message/Value/Xml); default `string` maps to `NVARCHAR2(2000)` which truncates.
- **No `ILIKE`** — use `EF.Functions.Like(col.ToUpper(), pattern.ToUpper())` for case-insensitive search.
- **Connection:** `User Id=VODONGHA_PERSONAL;Password=...;Data Source=vodongha_tp;` with the unzipped ADB wallet at `./wallet` (git-ignored) mounted to `/app/wallet` and `TNS_ADMIN=/app/wallet`. The wallet's `sqlnet.ora` must point `WALLET_LOCATION` at `/app/wallet`.
...
| Version | Changes |
|---|---|
| v3.0.6 (DB migration) | **Migrated database Neon PostgreSQL → Oracle Autonomous Database 19c** (shared ADB, schema `VODONGHA_PERSONAL`). EF provider `Npgsql` → `Oracle.EntityFrameworkCore` with `UseOracleSQLCompatibility(DatabaseVersion19)`; 51 Npgsql migrations replaced by a single `InitialOracle` (Guid→`RAW(16)`, long text→`NCLOB`, bool→`NUMBER(1)`); `EF.Functions.ILike`→`Like(UPPER(..))`; optional string props made nullable (Oracle stores `''` as NULL); wallet-mTLS connection (`./wallet`→`/app/wallet`, `TNS_ADMIN`); removed the Neon-only **Costs** feature + all Neon/Postgres references; deploy webhook now rebuilds `--no-cache` when dependency manifests change. 58,140 rows migrated via ETL. See **Oracle database notes**. |
